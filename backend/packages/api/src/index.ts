...
export const createApp = (deps: AppDependencies): Express => {
  ...
  const emailVerificationTokenRepository =
    new DrizzleEmailVerificationTokenRepository(database as PostgresJsDatabase)
  const authAuditRepository = new DrizzleAuthAuditRepository(
    database as PostgresJsDatabase
  )
  const failedLoginRepository = new DrizzleFailedLoginRepository(
    database as PostgresJsDatabase
  )
  ...
  const authUserRepository = {
    ...
    create: async (user: {
      ...
      const newUser = {
        ...
        data: {
          id: uuidv4() as UserId, // Generate UUID for new user
          email: user.email,
          name: user.name,
  ...
  app.use(
    ...
    createAuthRoutes({
      ...
      failedLoginRepository: {
        recordAttempt: async (email: string, ipAddress?: string) => {
          const result = await failedLoginRepository.recordAttempt(
            email,
            ipAddress
          )
          return result
        ...
        isAccountLocked: async (email: string) => {
          return await failedLoginRepository.isAccountLocked(email)
        },
        clearAttempts: async (email: string) => {
          return await failedLoginRepository.clearAttempts(email)
        },
        lockAccount: async (email: string, lockDurationMinutes: number) => {
          return await failedLoginRepository.lockAccount(
            email,
            lockDurationMinutes
          )
        },
      ...
      authAuditRepository: {
        log: async (entry) => {
          await authAuditRepository.log(entry as AuditLogEntry)
          return { type: 'ok' as const, value: undefined }
        },
      },
    })
  )
  ...
  app.use(
    ...
    createPasswordChangeRoutes({
      ...
      sendPasswordChangedEmail: async () => {
        // TODO: Implement password changed email
        return { type: 'ok' as const, value: undefined }
      },
      authConfig,
    })
