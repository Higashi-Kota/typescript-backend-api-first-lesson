...
export const createAuthRoutes = (deps: AuthRouteDeps): Router => {
   ...
   */
  router.post('/logout', authenticate(authConfig), async (req, res, next) => {
    try {
  ...
  router.post(
    ...
    async (req, res, next) => {
      try {
        ...
        res.json({
          secret: secretResult.value.secret,
          qrCode: secretResult.value.qrCode,
   ...
   */
  router.post('/2fa/verify', authRateLimiter, async (req, res, next) => {
    try {
      const schema = z.object({
   ...
   */
  router.get('/sessions', authenticate(authConfig), async (req, res, next) => {
    try {
      if (!req.user) {
        return res.status(401).json({
          code: 'UNAUTHORIZED',
